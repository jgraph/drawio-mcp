...
- **`vendor/libavoid/libavoid.min.js` + `libavoid.wasm`** (~44 KB glue + ~492 KB wasm) — libavoid-js, the obstacle-avoiding orthogonal edge router behind `routing: "libavoid"`. The glue is ESM + uses `import.meta.url`; `processLibavoidBundle` neutralizes that, patches the loader to take an inlined `wasmBinary`, and aliases `globalThis.AvoidLib`. The wasm is base64-inlined and fed as `wasmBinary` (no fetch — the sandbox has no allow-same-origin and the CSP forbids `data:` in connect-src). The routing math is `AvoidRouting.computeRoutes` from the vendored `vendor/libavoid/libavoid-routing.js` (appended to the processed glue and inlined with it) — a **verbatim copy** of the canonical `drawio-dev js/libavoid-js/libavoid-routing.js`, the same artifact the draw.io editor bundles and the mcp-tool-server imports. See `vendor/libavoid/README.md`. (Stays vendored — WASM can't be a plain CDN `<script src>`.)
